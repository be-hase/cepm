...
func runLoadCeremony(ctx context.Context, cmd *cobra.Command, targets []loadTarget) {
	...
	for i, t := range targets {
		...
		switch waitForLoaded(ctx, out, t) {
		case loadConfirmed:
...
func waitForLoaded(ctx context.Context, out io.Writer, t loadTarget) loadWaitResult {
	...
	for time.Now().Before(deadline) {
		...
		for _, e := range exts {
			...
			if !baseline[e.ID] && !warned {
				warned = true
				fmt.Fprintf(out, "\n  ⚠ %q was loaded, but from a different directory than expected.\n", e.Name)
