...
- If Chrome is closed, `cepm update` still pulls; on the next Chrome launch
  the host runs a catch-up reload, because Chrome may keep running cached
  code (service workers in particular) until told to re-read the disk.
...
This generates the helper extension into `~/.cepm/helper` and registers the
native messaging host with Chrome. Then load the helper — the one step
Chrome insists a human performs: open *chrome://extensions*, turn on
*Developer mode*, click *"Load unpacked"*, and pick `~/.cepm/helper`.
...
(With Chrome closed, "Chrome is not reachable" here is expected — everything
...
cepm clones the repo, finds every extension in it (any directory with a
`manifest.json`), asks which ones you want, and walks you through loading
each one: the directory path lands on your clipboard, chrome://extensions
opens, and cepm confirms once Chrome has actually loaded it.

**4. There is no step 4.** While Chrome runs, cepm pulls every hour and
reloads whatever changed; `cepm update` does the same on demand, and
`cepm list` shows what is registered and loaded.
...
Every extension cepm knows about is in one of three states, and it only
moves between them when you say so:

```
               cepm install (picked)            "Load unpacked", once
┌───────────┐  or cepm enable    ┌─────────┐    (cepm guides you)    ┌────────┐
│ available │ ─────────────────► │ enabled │ ──────────────────────► │ loaded │
└───────────┘ ◄───────────────── └─────────┘                         └────────┘
               cepm disable
```

- **available** — registered, nothing more. Extensions you skipped during
  `cepm install`, and ones the repo adds later, wait here: never loaded,
  reloaded, or nagged about until you opt in.
- **enabled** — you want it. cepm keeps it updated and expects it in Chrome;
  all that is missing is the one-time manual load.
- **loaded** — Chrome has it, and updates are hands-free from here on: a
  pull that changes its files reloads it immediately. The one exception is
  a `manifest.json` change, which Chrome only applies after a restart —
  cepm tells you when that happens.

Leaving is the same road backwards: `cepm disable` returns an extension to
*available* (and offers to remove it from Chrome), and `cepm uninstall
<repo>` unregisters the whole repository, moving its clone to a trash
directory rather than deleting it. If the *repo* renames or deletes an
extension directory, the Chrome entry left behind turns stale — `cepm
cleanup` removes it via Chrome's own confirmation dialog.
...
- The helper extension needs the `management` permission (to toggle other
  extensions), `nativeMessaging`, `alarms`, and `storage` (a crash-recovery
  marker around its own reloads, so a mid-reload service-worker death cannot
  leave an extension disabled). It never touches page data.
