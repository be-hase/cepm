...
`cepm list --all` (every status, one row per extension — the default is one
row per repo, showing only what is loaded plus a count of what it hides),
`cepm list --full` (all columns: EXTENSION, ID, DIR and URL),
`cepm list --status not-loaded,stale` (filter by status),
