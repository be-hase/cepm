...
The default is one row per loaded repo (plus a count of what it hides).
`--full` shows one row per extension, with the EXTENSION, ID, DIR and URL
columns; `--all` shows everything registered, including extensions you
skipped; `--status not-loaded,stale` filters by status; `--json` is for
scripts.
